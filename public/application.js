...
/***/ (function(module, exports, __webpack_require__) {
	...
	var printEvents = __webpack_require__(37);

	$(function () {
	  setAPIOrigin(location, config);
	  authEvents.addHandlers();
	  printEvents.addPrintHandlers();
	});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var printAPI = __webpack_require__(12);
	var printUI = __webpack_require__(13);
	...
	var onSignUp = function onSignUp(event) {
	  ...
	  api.signUp(data).then(ui.signUpSuccess).then(function () {
	    api.signIn(data).then(ui.signInSuccess).then(function () {
	      printAPI.indexPrints().then(printUI.indexPrintsSuccess).catch(printUI.indexPrintsFailure);
	    }).then(function () {
	      printAPI.showOrder().then(printUI.showOrderSuccess).catch(printUI.showOrderFailure);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var emptyCartSuccess = function emptyCartSuccess(data) {
	  store.buyerId = data.buyer._id;
	};

	var emptyCartFailure = function emptyCartFailure(response) {};
	...
	module.exports = {
	  ...
	  emptyCartFailure: emptyCartFailure,
	  emptyCartSuccess: emptyCartSuccess
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var titleArray = ['bento box', 'sushi roll', 'lego man', 'mini cactus', 'eclair', 'sriracha', 'terrarium', 'pineapple', 'platypus'];
	...
	var indexPrints = function indexPrints() {
	  return $.ajax({
	...
	var createPrint = function createPrint(data) {
	  return $.ajax({
	    url: config.apiOrigin + '/prints',
	    method: 'POST',
	    headers: {
	...
	var removeById = function removeById(printId) {
	  return $.ajax({
	...
	var updateById = function updateById(printId, data) {
	  return $.ajax({
	    url: config.apiOrigin + '/prints/' + printId,
	    method: 'PATCH',
	    headers: {
	...
	var makeCharge = function makeCharge(token) {
	  return $.ajax({
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var store = __webpack_require__(10);
	var showPrintsTemplate = __webpack_require__(14);
	var showPurchaseTemplate = __webpack_require__(35);
	var api = __webpack_require__(12);
	var getFormFields = __webpack_require__(8);

	var indexPrintsSuccess = function indexPrintsSuccess(response) {
	  store.indexOfPrints = response;
	  var indexPrintsHtml = showPrintsTemplate({
	...
	var calculateTotalCost = function calculateTotalCost(response) {
	  var printArray = store.indexOfPrints.prints;
	  var totalQuantity = printArray.reduce(function (a, b) {
	    return a + b.quantity;
	  }, 0);
	...
	var indexPrintsFailure = function indexPrintsFailure(response) {
	  $('.cartHas-display').text(response);
	};

	var removePrint = function removePrint(event) {
	  event.preventDefault();
	  var findId = $(event.target).attr('data-id');
	  api.removeById(findId).then(removePrintSuccess(event.target)).then(function () {
	    api.indexPrints().then(indexPrintsSuccess).catch(indexPrintsFailure);
	  }).catch(removeprintFailure);
	};

	var updatePrint = function updatePrint(event) {
	  event.preventDefault();
	  var findId = $(event.target).attr('data-id');
	  var data = getFormFields(event.target);
	  api.updateById(findId, data).then(updatePrintSuccess).then(function () {
	    api.indexPrints().then(indexPrintsSuccess).catch(indexPrintsFailure);
	  }).catch(updatePrintFailure);
	};

	var updatePrintSuccess = function updatePrintSuccess(response) {};

	var updatePrintFailure = function updatePrintFailure(response) {
	  $('.text-display').html('Error updating quantity');
	};

	var removePrintSuccess = function removePrintSuccess(target) {
	  $('.create-print-message').detach();
	  if ($('.create-print-failure').length) {
	    $('.create-print-failure').detach();
	  }
	};
	var removeprintFailure = function removeprintFailure(response) {
	...
	var showOrderSuccess = function showOrderSuccess(data) {
	  if (data.orders.length === 0) {
	    ...
	    var indexPurchaseHtml = showPurchaseTemplate({
	      orders: data.orders
	    });
	...
	var showOrderFailure = function showOrderFailure(response) {
	  $('.purchase-display').text('no purchase history to display');
	};

	var createPrintSuccess = function createPrintSuccess(target) {
	  if ($('.create-print-failure').length) {
	    $('.create-print-failure').detach();
	  }
	...
	var createPrintFailure = function createPrintFailure(target) {
	...
	var alreadyInCart = function alreadyInCart(target) {
	  if ($('.create-print-message').length) {
	    $('.create-print-message').detach();
	  }
	...
	var tokenFailure = function tokenFailure(response) {
	  $('.purchaseConfirm').text('unable to process purchase');
	};

	module.exports = {
	  showOrderFailure: showOrderFailure,
	  showOrderSuccess: showOrderSuccess,
	  tokenFailure: tokenFailure,
	  createPrintFailure: createPrintFailure,
	  createPrintSuccess: createPrintSuccess,
	  alreadyInCart: alreadyInCart,
	  indexPrintsFailure: indexPrintsFailure,
	  indexPrintsSuccess: indexPrintsSuccess,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "      <tr>\n        <td>"
	    ...
	    + "\" class=\"update-print-button\">\n            <fieldset>\n              <div class=\"col-xs-5\">\n                <input type=\"number\" class=\"form-control zoom-control\" name=\"print[quantity]\" placeholder= "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.quantity : stack1), depth0))
	    + " min=\"1\" max=\"10\">\n              </div>\n              <button type=\"submit\">update</button>\n              <button class=\"remove-print-button\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">remove</button>\n            </fieldset>\n          </form>\n        </td>\n        <!-- <td>\n          <button class=\"remove-print-button\"  data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">remove</button>\n        </td> -->\n        </div>\n      </tr>\n";
	  ...
	  return "<!-- "
	    ...
	    + " -->\n\n<table class=\"cart-handlebars\" data-id=\""
	    + container.escapeExpression(container.lambda(((stack1 = (depth0 != null ? depth0.print : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\">\n  <thead>\n    <tr>\n      <th>Print</th>\n      <th>Quantity</th>\n    </tr>\n  </thead>\n\n  <tbody>\n"
	    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.prints : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "  </tbody>\n</table>\n\n<div class=\"total-cost-display\"></div>\n";
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "    <tr>\n      <td>"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.printName : stack1), depth0))
	    + "</td>\n      <td>"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.printQuant : stack1), depth0))
	    + "</td>\n      <td>"
	    + ((stack1 = __default(__webpack_require__(36)).call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.createdAt : stack1),{"name":"formatDate","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
	    + "</td>\n    </tr>\n";
	  ...
	  return "<table class=\"history-handlebars\">\n  <thead>\n    <tr>\n      <th>Print</th>\n      <th>Quantity</th>\n      <th>Date</th>\n    </tr>\n  </thead>\n  <tbody>\n"
...
/***/ (function(module, exports) {
	...
	var formatDate = function formatDate(dateString, options) {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onGetCart = function onGetCart(event) {
	  event.preventDefault();
	  api.getCart().then(ui.getCartSuccess).catch(ui.getCartFailure);
	};

	var isPrintAlreadyInCart = function isPrintAlreadyInCart(printArray, printId) {
	  if (printArray.prints.length === 0) {
	    return isInArray;
	  }
	  var isInArray = false;
	  for (var i = 0; i < printArray.prints.length; i++) {
	...
	var onCreatePrint = function onCreatePrint(event) {
	  event.preventDefault();
	  var printArray = store.indexOfPrints;
	  var data = getFormFields(event.target);
	  // stores unique id for later use
	  var printId = $(event.target).attr('data-id');
	  ...
	  if (isPrintAlreadyInCart(printArray, printId)) {
	    ...
	    api.createPrint(data).then(function () {
	      ui.createPrintSuccess(event.target);
	    }).catch(function () {
	      ui.createPrintFailure(event.target);
	    }).then(function () {
	      api.indexPrints().then(ui.indexPrintsSuccess).catch(ui.indexPrintsFailure);
	    });
	  }
	};

	var onUpdatePrint = function onUpdatePrint(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  var idNum = $(event.target).attr('data-id');
	  store.printId = idNum;
	  api.updatePrint(data).then(ui.updatePrintSuccess).catch(ui.updatePrintFailure);
	};

	var onShowOrder = function onShowOrder() {
	  $('.purchase-display').show();
	  event.preventDefault();
	  api.showOrder().then(ui.showOrderSuccess).catch(ui.showOrderFailure);
	};

	var onIndexPrints = function onIndexPrints() {
	  $('.cartHas-display').show();
	  event.preventDefault();
	  api.indexPrints().then(ui.indexPrintsSuccess).then().catch(ui.indexPrintsFailure);
	};
	...
	var handleToken = function handleToken(token) {
	  ...
	  api.makeCharge(token).then(function (output) {
	    var cost = (output.amount / 100).toString();
	...
	var onCheckout = function onCheckout(ev) {
	...
	var onCreateOrder = function onCreateOrder() {
	  var prints = store.indexOfPrints;
	  for (var i = 0; i < prints.prints.length; i++) {
	    var data = {
	      'order': {
	    ...
	    api.createOrder(data).then(ui.createOrderSuccess).catch(ui.createOrderFailure);
	  }
	};

	var onRemovePrints = function onRemovePrints() {
	  var prints = store.indexOfPrints;
	  for (var i = 0; i < prints.prints.length; i++) {
	    var findId = prints.prints[i].id;
	    api.removeById(findId).then(ui.removePrintsSuccess).then(function () {
	      api.indexPrints().then(ui.indexPrintsSuccess).catch(ui.indexPrintsFailure);
	    }).catch(ui.removePrintsFailure);
	  }
	};

	var addPrintHandlers = function addPrintHandlers() {
	...
	module.exports = {
	  addPrintHandlers: addPrintHandlers
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	// module
	exports.push([module.id, "/* http://meyerweb.com/eric/tools/css/reset/\n   v2.0 | 20110126\n   License: none (public domain)\n*/\nhtml, body, div, span, applet, object, iframe,\nh1, h2, h3, h4, h5, h6, p, blockquote, pre,\na, abbr, acronym, address, big, cite, code,\ndel, dfn, em, img, ins, kbd, q, s, samp,\nsmall, strike, strong, sub, sup, tt, var,\nb, u, i, center,\ndl, dt, dd, ol, ul, li,\nfieldset, form, label, legend,\ntable, caption, tbody, tfoot, thead, tr, th, td,\narticle, aside, canvas, details, embed,\nfigure, figcaption, footer, header, hgroup,\nmenu, nav, output, ruby, section, summary,\ntime, mark, audio, video {\n  margin: 0;\n  padding: 0;\n  border: 0;\n  font-size: 100%;\n  font: inherit;\n  vertical-align: baseline; }\n\n/* HTML5 display-role reset for older browsers */\narticle, aside, details, figcaption, figure,\nfooter, header, hgroup, menu, nav, section {\n  display: block; }\n\nbody {\n  line-height: 1; }\n\nol, ul {\n  list-style: none; }\n\nblockquote, q {\n  quotes: none; }\n\nblockquote:before, blockquote:after,\nq:before, q:after {\n  content: '';\n  content: none; }\n\ntable {\n  border-collapse: collapse;\n  border-spacing: 0; }\n\nbody,\nhtml {\n  background: #FFF0F5;\n  color: #BFB39D;\n  font-family: 'PT Serif', serif;\n  font-size: 16px; }\n\nnav {\n  position: fixed;\n  left: 0;\n  top: 0;\n  height: 58px;\n  width: 100%;\n  text-align: center;\n  z-index: 1000;\n  background-color: #FFF0F5;\n  border-bottom: 2px solid #4c4339; }\n\n.btn-transparent {\n  background: transparent;\n  color: #524c42;\n  font-weight: 600;\n  -webkit-transition: background 0.2s ease-in-out, border 0.2s ease-in-out;\n  -moz-transition: background 0.2s ease-in-out, border 0.2s ease-in-out;\n  -o-transition: background 0.2s ease-in-out, border 0.2s ease-in-out;\n  -webkit-transition: background 0.2s ease-in-out, border 0.2s ease-in-out;\n  transition: background 0.2s ease-in-out, border 0.2s ease-in-out; }\n\n.btn-transparent:focus {\n  background: transparent;\n  outline: none;\n  color: #524c42; }\n\n.btn-transparent:hover {\n  background: transparent;\n  color: #272b47; }\n\n.btn-transparent:active {\n  outline: none; }\n\ninput {\n  font-size: 16px; }\n\n.zoom-control {\n  font-size: 16px; }\n\n.modal-body,\n.modal-content,\n.modal-footer,\n.modal-header {\n  border: 0;\n  background-color: #FFF0F5;\n  text-align: center; }\n\n.modal-header,\n.modal-footer {\n  border-radius: 10px; }\n\n.left {\n  float: left; }\n\n.right {\n  float: right; }\n\n.transformer {\n  transform: translate(-50%, 0); }\n\nheader {\n  position: relative;\n  height: 40vh;\n  text-align: center;\n  overflow: hidden; }\n\nheader h1 {\n  border-radius: 10px;\n  color: #403C34;\n  display: inline-block;\n  font-size: 70px;\n  margin-top: 10vh;\n  opacity: 0.9;\n  padding: 1rem;\n  position: relative;\n  z-index: 15; }\n\nh1 {\n  font-size: 4rem; }\n\nh2 {\n  font-size: 1.5rem; }\n\nh4 {\n  color: #524c42;\n  font-size: 1rem; }\n\ninput {\n  font-size: 16px; }\n\n.hidden {\n  display: none; }\n\n.paintbrush,\n.palette {\n  position: absolute;\n  z-index: 5; }\n\n.paintbrush {\n  height: 90%;\n  left: 0; }\n\n.palette {\n  height: 40vh;\n  right: 0;\n  transition: opacity 0.2s ease; }\n\n.alreadyPurchased,\n.cartHas {\n  text-align: center; }\n\n.store-display {\n  background: #272B47;\n  padding: 2rem 0;\n  text-align: center; }\n\n.print-container {\n  display: inline-block;\n  margin: 2rem;\n  width: 26vw; }\n\n.stripe-checkout {\n  text-align: center; }\n\n.print-image-container {\n  display: flex;\n  justify-content: center;\n  align-items: center;\n  background: #ffffff;\n  height: 26vw;\n  margin-bottom: 1rem;\n  border: 1rem solid #FFF0F5; }\n\n.print-image {\n  max-height: 25vw;\n  max-width: 25vw; }\n\n.create-print-failure,\n.create-print-message {\n  position: absolute;\n  color: #FFF0F5;\n  left: 50%;\n  width: 100%;\n  padding: 0.5rem 0; }\n\n.create-print-failure p,\n.create-print-message p {\n  color: #FFF0F5;\n  position: relative;\n  left: -50%;\n  width: 100%; }\n\nbutton,\nform {\n  position: relative;\n  z-index: 2; }\n\ninput {\n  outline: none;\n  padding: 0.2rem 0;\n  margin-right: 0.7rem; }\n\nbutton:focus {\n  outline: 0; }\n\n.create-print-failure,\n.create-print-message,\n.total-cost-display,\np {\n  font-family: 'Open Sans', sans-serif;\n  margin-bottom: 0.5rem;\n  color: #524c42; }\n\n.total-cost-display {\n  font-family: 'Open Sans', sans-serif;\n  margin-bottom: 0.5rem;\n  color: #524c42;\n  text-align: center; }\n\nul.dropdown-menu.pull-right {\n  background-color: #FFF0F5;\n  margin-top: 3rem; }\n\nul.dropdown-menu {\n  background-color: #FFF0F5;\n  height: auto; }\n\n.hideOnStart {\n  display: none; }\n\n.text-display {\n  position: relative;\n  font-size: 1.5rem;\n  color: #4c443a;\n  z-index: 40; }\n\n.cartHas-display,\n.purchase-display,\n.purchaseConfirm {\n  font-size: 1rem;\n  color: #4c443a;\n  padding: 6px 0; }\n\n.purchaseConfirm {\n  padding-bottom: 12px; }\n\n#confirm-purchase {\n  text-align: center;\n  font-size: 1.3rem; }\n\nbutton {\n  background-color: #668578;\n  color: white;\n  -webkit-transition-duration: 0.4s;\n  border: none;\n  border-radius: 5px;\n  transition-duration: 0.4s;\n  margin: .3rem; }\n\nbutton:hover {\n  background-color: #555; }\n\nbutton:focus {\n  outline: 0; }\n\n#buttonSignIn,\n#buttonSignUp,\n#buttonPassword,\n#buttonSignOut {\n  background-color: #272b47; }\n\n.alertDisp {\n  text-align: center;\n  color: #524c42; }\n\n.print-name {\n  color: #FFF0F5;\n  font-size: 16px;\n  padding-top: 0.6rem; }\n\n.padMe {\n  padding-top: 8px; }\n\n.dropdown-menu {\n  left: 50%;\n  right: auto;\n  text-align: center; }\n\n.button.btn.btn-transparent.btn-lg.dropdown-toggle {\n  margin-top: .36em;\n  font-size: 18px; }\n\n.addMarg {\n  margin: 10px; }\n\n.choose-quantity {\n  min-width: 3.4rem; }\n\n.cart-handlebars {\n  font-family: 'Open Sans', sans-serif;\n  margin: 25px auto;\n  border-collapse: collapse;\n  border: 1px solid #D3D3D3;\n  box-shadow: 0 0 20px rgba(0, 0, 0, 0.1), 0 10px 20px rgba(0, 0, 0, 0.05), 0 20px 20px rgba(0, 0, 0, 0.05), 0 30px 20px rgba(0, 0, 0, 0.05); }\n  .cart-handlebars tr:hover {\n    background: #f4f4f4; }\n    .cart-handlebars tr:hover td {\n      color: #555; }\n  .cart-handlebars td,\n  .cart-handlebars th {\n    border: 1px solid #D3D3D3;\n    border-collapse: collapse;\n    color: #999;\n    padding: 10px; }\n  .cart-handlebars th {\n    background: #668578;\n    color: #fff;\n    text-transform: uppercase;\n    font-size: 12px; }\n    .cart-handlebars th.last {\n      border-right: none; }\n\n.history-handlebars {\n  width: 100%;\n  font-family: 'Open Sans', sans-serif;\n  margin: 25px auto;\n  border-collapse: collapse;\n  border: 1px solid #D3D3D3;\n  box-shadow: 0 0 20px rgba(0, 0, 0, 0.1), 0 10px 20px rgba(0, 0, 0, 0.05), 0 20px 20px rgba(0, 0, 0, 0.05), 0 30px 20px rgba(0, 0, 0, 0.05); }\n  .history-handlebars tr:hover {\n    background: #f4f4f4; }\n    .history-handlebars tr:hover td {\n      color: #555; }\n  .history-handlebars td,\n  .history-handlebars th {\n    border: 1px solid #D3D3D3;\n    border-collapse: collapse;\n    color: #999;\n    padding: 10px; }\n  .history-handlebars th {\n    background: #668578;\n    color: #fff;\n    text-transform: uppercase;\n    font-size: 12px; }\n    .history-handlebars th.last {\n      border-right: none; }\n\n.history-handlebars td {\n  text-align: left; }\n\n@media (max-width: 560px) {\n  #responsive {\n    font-size: 35px; }\n  ul.dropdown-menu.pull-right {\n    width: 100%; }\n  .btn.btn-transparent {\n    font-size: 17px; }\n  .addMarg {\n    margin: auto; } }\n\n.create-print-failure,\n.create-print-message {\n  font-size: .8rem; }\n\n@media all and (max-width: 750px) {\n  .palette {\n    opacity: 0; } }\n\n@media only screen and (min-device-width: 768px) and (max-device-width: 1024px) {\n  .palette {\n    opacity: 0; } }\n", ""]);
